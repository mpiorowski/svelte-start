<script lang="ts">
	...
	let selected = value ? value.split(' | ').map((v) => options.indexOf(v)) : [];
...
-->
<div>
	<label id="{name}-label" class="block text-sm font-medium leading-6" for={name}>
		{label}
	</label>
		...
		{#if open}
			...
			>
				{#each options as option, i}
                    ...
                    {active === i ? 'bg-indigo-600 text-white' : 'text-gray-900'}"
						id="{name}-option-{i}"
						role="option"
					...
					>
                        ...
                        {selected.includes(i) ? 'font-semibold' : 'font-normal'}"
						>
							{option}
						</span>
						{#if selected.includes(i)}
